...
# submatrices
Mkvl = Matrices.BuildMkvl(N) 
Mkcl = Matrices.BuildMkcl(N)
G    = zeros(N,N)
# G    = Matrices.BuildConductanceMatrix(N)
R    = Matrices.BuildResistanceMatrix(N)

# Thermal/electrical linear matrix
linThermMat = sparse(0.001.*Matrices.BuildHeatTransMatrix(N,dx))
...
# Mass Matrix
MassThermal    = sparse(Matrices.BuildThermalMassMatrix(N))
...
function HeatEq!(res,du, u, p, t)
    ...
    res[:] += [zeros(N-1);zeros(N-1); r.*JouleHeatFlowMatrix*(I.^2)]
